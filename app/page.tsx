...
function Splash({ onFim }: any) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", width: "400px", height: "400px", borderRadius: "50%", border: `1px solid rgba(255,255,255,0.08)`, top: "50%", left: "50%", transform: "translate(-50%, -50%)" }} />
      <div style={{ position: "absolute", width: "600px", height: "600px", borderRadius: "50%", border: `1px solid rgba(255,255,255,0.05)`, top: "50%", left: "50%", transform: "translate(-50%, -50%)" }} />
      ...
      <div style={{ opacity: fase >= 2 ? 1 : 0, transform: fase >= 2 ? "translateY(0)" : "translateY(20px)", transition: "all 0.6s ease", textAlign: "center" }}>
        <h1 style={{ color: cores.branco, fontSize: "36px", fontWeight: "800", margin: 0, letterSpacing: "6px", textTransform: "uppercase" }}>ARTEMIS</h1>
        <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "13px", letterSpacing: "4px", marginTop: "6px", textTransform: "uppercase" }}>REACH</p>
      </div>

      <div style={{ opacity: fase >= 3 ? 1 : 0, transition: "opacity 0.4s ease", display: "flex", gap: "8px", marginTop: "48px" }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{ width: "8px", height: "8px", borderRadius: "50%", backgroundColor: "rgba(255,255,255,0.6)", animation: `pulse 1.2s ease-in-out ${i * 0.2}s infinite` }} />
        ))}
      </div>
...
export default function Home() {
  ...
  async function cadastrar() {
    setErro("")
    if (!nome || !email || !senha || !telefone) { setErro("Preencha todos os campos."); return }
    setCarregando(true)
    try {
      const cred = await createUserWithEmailAndPassword(auth, email, senha)
      await setDoc(doc(db, "usuarios", cred.user.uid), {
        nome, email, telefone, criado_em: new Date().toISOString()
      })
      setTela("modo")
    } catch {
  ...
  function escolherModo(modo: string) {
    if (modo === "echo") {
          ...
          >
            ...
            {item.destaque && (
              <div style={{
                ...
                color: cores.roxo, fontSize: "11px", fontWeight: "700",
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: cores.branco, borderRadius: "20px", padding: "32px", width: "100%", maxWidth: "440px", boxShadow: "0 4px 24px rgba(90,73,151,0.12)" }}>
        {tela === "cadastro" && <Campo label="Nome completo" placeholder="Seu nome completo" value={nome} onChange={setNome} tipo="text" />}
        ...
        <Campo label="Senha" placeholder="••••••••" value={senha} onChange={setSenha} tipo={mostrarSenha ? "text" : "password"}
          extra={<span onClick={() => setMostrarSenha(!mostrarSenha)} style={{ cursor: "pointer", color: cores.roxo, fontSize: "13px" }}>{mostrarSenha ? "Ocultar" : "Ver"}</span>}
        />
        {tela === "cadastro" && <Campo label="Telefone" placeholder="(00) 00000-0000" value={telefone} onChange={setTelefone} tipo="tel" />}
        ...
        <p style={{ textAlign: "center", fontSize: "13px", color: "#888", margin: 0 }}>
          ...
          <span onClick={() => { setTela(tela === "login" ? "cadastro" : "login"); setErro("") }} style={{ color: cores.roxo, fontWeight: "bold", cursor: "pointer" }}>
...
function Campo({ label, placeholder, value, onChange, tipo, extra = null }: any) {
  return (
    <div style={{ marginBottom: "20px" }}>
      <label style={{ fontSize: "13px", fontWeight: "600", color: "#2F195F", display: "block", marginBottom: "8px" }}>{label}</label>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", border: "1.5px solid #E8E0F5", borderRadius: "12px", padding: "12px 16px" }}>
        <input type={tipo} placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)}
