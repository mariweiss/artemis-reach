    ...
    const unsub = onSnapshot(q, (snap) => {
      const locs = snap.docs.map((d) => {
        ...
        const grupoDoMembro = grupos.find(
          (g) => gruposSelecionados.has(g.id) && (g.membros || []).includes(data.usuario_id)
        )
  ...
  return (
    <div style={{ fontFamily: "sans-serif", backgroundColor: cores.fundo }}>
      ...
      <div style={{ position: "fixed", bottom: "90px", left: "24px" }}>
        <button onClick={() => minhaPos && setMinhaPos({ ...minhaPos })} style={{
      ...
      {modalGrupos && (
        <>
          <div onClick={() => setModalGrupos(false)} style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <div>
                ...
                <p style={{ color: cores.lavanda, margin: "4px 0 0", fontSize: "12px" }}>
                  Selecione quais grupos você quer visualizar
                </p>
              </div>
            ...
            {grupos.length === 0 ? (
              <div style={{ textAlign: "center", padding: "24px 0" }}>
                <p style={{ color: cores.lavanda, fontSize: "14px" }}>Você ainda não tem grupos criados.</p>
                <p style={{ color: "#bbb", fontSize: "12px" }}>Crie grupos na aba Círculo para visualizá-los aqui.</p>
              </div>
            ) : grupos.map((grupo) => {
